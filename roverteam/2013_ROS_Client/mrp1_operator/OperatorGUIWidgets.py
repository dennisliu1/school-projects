...
class DriveBarWidget(QWidget):
	...
	def __init__(self, width,height, ruler_tick_size, ruler_tick_length, tick_large_length, ruler_tick_num, large_tick_offset, parent = None):
...
class RoverTiltWidget(QLabel):
	...
	class RoverCameraWidget(QWidget):
		...
		def _drawPic(self, qp):
			...
			length = (-(self.height/2-10))*((1-(angle)/90.0))
			qp.drawRect(self.width/2-2,self.height/2,5,length)
...
class HelpWidget(QWidget):
	...
	def drawHelp(self, qp):
		...
		qp.drawText(          0, 1*self.font_size*1.5, "Shortcuts:")
		qp.drawText(self.margin, 2*self.font_size*1.5, "F1 : Play Main Camera")
		qp.drawText(self.margin, 3*self.font_size*1.5, "F2 : Pause Main Camera")
		qp.drawText(self.margin, 4*self.font_size*1.5, "F3 : Stop Main Camera")
		qp.drawText(self.margin, 5*self.font_size*1.5, "F4 : Set Main Camera to selection")
		qp.drawText(self.margin, 6*self.font_size*1.5, "F5 : Play PIP Camera")
		qp.drawText(self.margin, 7*self.font_size*1.5, "F6 : Pause PIP Camera")
		qp.drawText(self.margin, 8*self.font_size*1.5, "F7 : Stop PIP Camera")
		qp.drawText(self.margin, 9*self.font_size*1.5, "F8 : Set PIP Camera to selection")
		qp.drawText(self.margin,10*self.font_size*1.5, "F9 : Prev Camera Selection")
		qp.drawText(self.margin,11*self.font_size*1.5, "F10: Next Camera Selection")
		qp.drawText(self.margin,12*self.font_size*1.5, "F12: take screenshot")
		qp.drawText(self.margin,13*self.font_size*1.5, "\\  : hide HUD")
		qp.drawText(self.margin,14*self.font_size*1.5, "G  : expand Map")
		qp.drawText(self.margin,15*self.font_size*1.5, "M  : change map")
		qp.drawText(self.margin,16*self.font_size*1.5, "N  : science heading mode")
		qp.drawText(self.margin,17*self.font_size*1.5, "-  : zoom out map")
		qp.drawText(self.margin,18*self.font_size*1.5, "+  : zoom in  map")
		qp.drawText(self.margin,19*self.font_size*1.5, "C  : change task list display(1/2/3)")
		qp.drawText(self.margin,20*self.font_size*1.5, "/  : This help menu")
		qp.drawText(self.margin,21*self.font_size*1.5, "Esc: Exit GUI")

		# qp.drawText(10,19*20, "Z  : done current task")
		# qp.drawText(10,20*20, "X  : undo previous task")
...
class GPSDisplayWidget(QWidget):
	Signal_Update_GPS = pyqtSignal(float,float)
	...
	def _Slot_Update_GPS(self, longitude,latitude):
		self._draw_GPS = str(longitude)+":"+str(latitude)
		self.repaint()

class AstronautBoxWidget(QWidget):
	Signal_Dump_Astrobox = pyqtSignal(int, float)
	def __init__(self, width,height,font, parent=None):
		super(AstronautBoxWidget, self).__init__(parent)
		self.width = width
		self.height = height
		self.resize(self.width, self.height)
		self.Signal_Dump_Astrobox.connect(self._Slot_Dump_Astrobox)
		self._BoxStatus = [False,False,True,False,False]
		self._color_Edge = QColor(255,0,0,200)
		self._color_Background_Undeployed = QColor(255,0,0,100)
		self._color_Background_Deployed = QColor(0,255,0,100)
		self._color_Text = QColor(255,255,255)
		self._font_width  = font
		self._font_height = self._font_width
		self.boxLength = (self.width/5.0)
	...
	def drawText(self, qp):
		...
		for i in range(len(self._BoxStatus)):
			if self._BoxStatus[i]:
				qp.setBrush(self._color_Background_Deployed)
			else:
				qp.setBrush(self._color_Background_Undeployed)
			qp.setPen(self._color_Edge)
			qp.drawRect(self.boxLength*i,0, self.boxLength,self.height-1)

			qp.setPen(self._color_Text)
			qp.setFont(QFont('Decorative', self._font_height))
			qp.drawText(self.boxLength*i+self.boxLength/4,self._font_height*1.5, str(i))

	def _Slot_Dump_Astrobox(self, index, speed):
		if speed != 0:
			self._BoxStatus[index] = True
		else:
			self._BoxStatus[index] = False
		self.repaint()
...
class MinimapWidget(QWidget):
	...
	#-------------------------------            Load Maps            -------------------------------
		...
		#NE,SW
		self._topoQuads.append([QImage(path+'/resources/q3128_DRG24k-c.tiff'), -110.87571203540497, -110.75070541499849,38.37498125909681, 38.49998105277704, 4485.0,5697.0])
		self._topoQuads.append([QImage(path+'/resources/q3128_NED10.tiff'), -110.87571203540497, -110.75070541499849,38.37498125909681, 38.49998105277704, 1188.0,1511.0])
		...
		# self._topoQuads.append([QImage(path+'/resources/hanksville/q3229_DRG24k-c.tif'), -110.75070841581692, -110.62570298853876, 38.24998330999908, 38.37498247798552, 4501,5703])
		# self._topoQuads.append([QImage(path+'/resources/hanksville/12SWH240440.tif'), -110.7254, -110.6794,38.343699999999544, 38.379899999999544, 16000,16000])
		'''
		
		'''
		#self._topoQuads.append([QImage(path+'/resources/hanksville/q3129_NED10.tiff'), -110.87571203540497, -110.75070541499849,38.37498125909681, 38.49998105277704, 1602,1597])
		self._PIXEL_COORD = []
	...
	#-------------------------------            Signals & Slots            -------------------------------
		...
		self._color_navpt = QColor(0,0,255)
	...
	def paintEvent(self, event):
		...
		qp.setPen(self._color_font)
		qp.setBrush(Qt.NoBrush)
		pixel = self._topoQuads[1][0].pixel(self._midCoordPixel[0],self._midCoordPixel[1])
		'''
		# High 1538.22m
		# Low: 1320.82m
		1538.22-1320.82 = 217.4
		'''
		pixel = 1320.82 + (217.4 * (pixel/4294967295.0))
		qp.drawText(10,10, str(self._zoom)+",H"+str(self._rover_heading)+"E"+str(pixel))
...
class VideoDisplayWidget(QWidget):
	...
	def paintEvent(self, event):
		...
		#painter.drawPixmap(0,0, self.width,self.height, self.pipelines._pixmaps[self._cameraIndex])
		painter.end()

	def Slot_Camera_Set(self, index):
		#print 'Slot_Camera_Slot:',index
		#self.Slot_Set_Camera(0)
		self._cameraIndex = index

	def setPixmap(self, pixmap2):
		self.pixmap = pixmap2
...
class GStreamerPipeline():
	...
	def startup(self, num):
		...
		for i in range(self.numberCameras):
			...
			self.videoSets[i][1]._Signal_Buffer.signal.connect(self.Slot_Trigger_Do_Render)
			#self.videoControls.videoSets[self.videoControls.self._length][1] # sink

	def addPipeline(self):
		port = self.START_PORT+self._length
		#strPipeline = "autovideosrc ! videorate ! videoscale ! video/x-raw-rgb,width=480,height=360 ! jpegenc ! VideoSink name=\"videosink0\" sync=false"
		#strPipeline = "v4l2src device=\"/dev/video"+str(self._length)+"\" ! videorate ! video/x-raw-rgb ! jpegenc ! VideoSink name=\"videosink0\" sync=false"
		#strPipeline = "rtspsrc location=\"rtsp://192.168.11.32:"+str(port)+"/video"+str(self._length)+"\" ! rtph264depay  ! h264parse ! ffdec_h264 ! queue ! jpegenc ! VideoSink name=\"videosink0\" sync=false"
		strPipeline = "rtspsrc location=\"rtsp://192.168.2.2:"+str(port)+"/video"+str(self._length)+"\" ! rtph264depay ! h264parse ! ffdec_h264 ! queue ! jpegenc ! VideoSink name=\"videosink0\" sync=false"
