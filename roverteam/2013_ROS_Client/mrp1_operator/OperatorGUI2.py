...
from mrp1_operator.msg import GPSpt,Conveyor,Drive,Navpt,Navpt_list,Navroute,Navroute_list,Task,Task_list,Timer,Compass,Ping,Signals,OperatorGUI_Status
#import GPSpt
'''
...
class ROSThread(QThread):
	...
	def __init__(self, parent=None):
		...
		self.sub_ALES_Drive = rospy.Subscriber("ALES_send_drive", Drive, self.get_ALES_drive)
		self.sub_ALES_GPS = rospy.Subscriber("ALES_Send_GPS", GPSpt, self.get_ALES_GPS)
		...
		self.sub_BASE_Drive = rospy.Subscriber("BASE_drive", Drive, self.get_BASE_drive)
	...
	def get_ALES_drive(self, drive):
		self.Signal_ALES_Drive.emit(drive.left,drive.right)
		pass
...
class OperatorGUI(QMainWindow):
	...
	def initUI(self):
		...
		self.videoDisplayWidget = VideoDisplayWidget(self.OperatorGUI_Window_Size.width(), self.OperatorGUI_Window_Size.height(), self.NUMBER_CAMERAS, self)
		...
		# self.PIPWidget_ratio = [8.0,6.0]
		# width = self.rPixel[0]*self.PIPWidget_ratio[0]
		# height = self.rPixel[1]*self.PIPWidget_ratio[1]
		# self.PIPWidget = VideoDisplayWidget(width,height, self.NUMBER_CAMERAS, self)
		# self.PIPWidget.move(self.OperatorGUI_Window_Size.width()-self.PIPWidget.width,self.videoControlWidget.height)
		...
		self.signalQualityWidget_ratio = [9.0,1.0]
		width = self.rPixel[0]*self.signalQualityWidget_ratio[0]
		height = self.rPixel[1]*self.signalQualityWidget_ratio[1]
		self.signalQualityWidget = SignalQualityWidget(width,height, self)
		self.signalQualityWidget.move(self.minimapWidget.width+self.roverTiltWidget.width+self.GPSDpWidget.width,self.OperatorGUI_Window_Size.height()-self.signalQualityWidget.height)
		#self.signalQualityWidget

		#optional widgets
		width = self.OperatorGUI_Window_Size.width()
		length = self.OperatorGUI_Window_Size.width()
		self.helpWidget = HelpWidget(self.OperatorGUI_Window_Size.width()/4, self.OperatorGUI_Window_Size.height()/4, self)
		...
		self.minimapWidget.Signal_Update_Camera_Heading_X.emit(self.rover_Telemetry_Camera_Heading)
		...
		self.rover_Telemetry_GPS_longitude  = self.minimapWidget._longitude#x; init as middle of map
		self.rover_Telemetry_GPS_latitude   = self.minimapWidget._latitude#y; init as middle of map
		...
		if cameraStatus == 0:# no msg(default)
			...
			if self.videoDisplayWidget._cameraIndex != self.currentMenuCamera:#change to proper camera
				...
				if self.videoDisplayWidget.currentVideoStatus == 0: # Stop: set to play
					...
					self.changeCameraStatus(1)
					print 'pause video'
			pass
		# if PIPStatus == 1:
		# 	print 'set PIPStatus'
		# 	if self.PIPWidget._cameraIndex != self.currentMenuCamera:#change to proper camera
		# 		print 'change PIP',self.PIPWidget._cameraIndex,self.currentMenuCamera
		# 		self.changePIPStatus(0)
		# 		self.PIPWidget._cameraIndex = self.currentMenuCamera
		# 		self.setPIPCamera(self.PIPWidget._cameraIndex)
		# 	elif self.PIPWidget._cameraIndex == self.currentMenuCamera:
		# 		if self.PIPWidget.currentVideoStatus == 0: # Stop: set to play
		# 			self.changePIPStatus(3)
		# 			print 'play video'
		# 		elif self.PIPWidget.currentVideoStatus == 1: # pause: set to play
		# 			self.changePIPStatus(3)
		# 			print 'play video'
		# 		elif self.PIPWidget.currentVideoStatus == 3: # play: set to pause
		# 			self.changePIPStatus(1)
		# 			print 'pause video'
	...
	def nextMenuCamera(self):
		if self.currentMenuCamera < self.videoDisplayWidget.numberCameras-1:
			#print 'menu next'
			self.currentMenuCamera += 1
			self.videoControlWidget.Signal_MenuCamera_Set.emit("video"+str(self.currentMenuCamera))
			#self.videoControlWidget.repaint()
	...
	def setMenuCamera(self, index):
		if 1 <= self.currentMenuCamera < self.videoDisplayWidget.numberCameras-1:
	...
	def nextCamera(self):
		if self.videoDisplayWidget._cameraIndex < self.videoDisplayWidget.numberCameras-1:
			self.videoDisplayWidget.Signal_Camera_Set.emit(self.videoDisplayWidget._cameraIndex+1)
			self.videoControlWidget.Signal_Camera_Set.emit("video"+str(self.videoDisplayWidget._cameraIndex))
	def setCamera(self, num):
		print 'set Camera(current)', self.videoDisplayWidget._cameraIndex
		if 0 <= self.videoDisplayWidget._cameraIndex < self.videoDisplayWidget.numberCameras:
			self.videoDisplayWidget.Signal_Camera_Set.emit(num)
			self.videoControlWidget.Signal_Camera_Set.emit("video"+str(self.videoDisplayWidget._cameraIndex))
			self.videoControlWidget.repaint()
	# def setPIPCamera(self, num):
	# 	print 'set PIP(current)', self.PIPWidget._cameraIndex
	# 	if 0 <= self.PIPWidget._cameraIndex < self.PIPWidget.numberCameras:
	# 		self.PIPWidget.Signal_Camera_Set.emit(num)
	# 		self.videoControlWidget2.Signal_Camera_Set.emit("video"+str(self.PIPWidget._cameraIndex))
	# 		self.videoControlWidget2.repaint()
			
	def changeCameraStatus(self, num):
		self.videoDisplayWidget.Signal_Set_Camera.emit(num)
